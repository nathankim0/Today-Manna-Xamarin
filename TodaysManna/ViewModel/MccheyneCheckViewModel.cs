...
namespace TodaysManna.ViewModel
{
    public class MccheyneCheckViewModel : PageBaseViewModel
    {
        ...
        {
            MccheyneCheckList.ForEach(async x =>
            {
                for(int i = 0; i < 5; i++)
                {
                    //x.Ranges[i].IsChecked = Preferences.Get(x.Ranges[i]?.Id, false);

                    var ischecked  = await App.ChecklistDatabase.GetItemIsChecked(x.Ranges[i]?.Id);
                    x.Ranges[i].IsChecked = ischecked;
                }
        ...
        private void OnCheckButtonTapped(string val)
        {
            ...
            MccheyneCheckList.ForEach(async x =>
            {
                var item = new MccheyneCheckItem();
                ...
                if (x.Ranges[0].Id == val)
                {
                    ...
                    item.CheckIndex = x.Ranges[0].Id;
                    item.IsChecked = x.Ranges[0].IsChecked;
                    await App.ChecklistDatabase.SaveItemAsync(item);
                }
                else if (x.Ranges[1].Id == val)
                {
                    ...
                    item.CheckIndex = x.Ranges[1].Id;
                    item.IsChecked = x.Ranges[1].IsChecked;
                    await App.ChecklistDatabase.SaveItemAsync(item);
                }
                else if (x.Ranges[2].Id == val)
                {
                    ...
                    item.CheckIndex = x.Ranges[2].Id;
                    item.IsChecked = x.Ranges[2].IsChecked;
                    await App.ChecklistDatabase.SaveItemAsync(item);
                }
                else if (x.Ranges[3].Id == val)
                {
                    ...
                    item.CheckIndex = x.Ranges[3].Id;
                    item.IsChecked = x.Ranges[3].IsChecked;
                    await App.ChecklistDatabase.SaveItemAsync(item);
                }
                else if (x.Ranges[4].Id == val)
                {
                    ...
                    item.CheckIndex = x.Ranges[4].Id;
                    item.IsChecked = x.Ranges[4].IsChecked;
                    await App.ChecklistDatabase.SaveItemAsync(item);
                }
            });
        }
